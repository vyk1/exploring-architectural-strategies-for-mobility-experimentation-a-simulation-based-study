...
public class MicroservicesController extends SimEntity {
	 ...
	 * @param applications
	 */
	public MicroservicesController(String name, List<FogDevice> fogDevices, List<Sensor> sensors,
			List<Application> applications, List<Integer> clusterLevels, Double clusterLatency, int placementLogic) {
		super(name);
	...
	protected void initializeControllers(int placementLogic) {
		for (FogDevice device : fogDevices) {
			...
			if (cdevice.getDeviceType().equals(MicroserviceFogDevice.FON)
					|| cdevice.getDeviceType().equals(MicroserviceFogDevice.CLOUD)) {
				List<FogDevice> monitoredDevices = getDevicesForFON(cdevice);
				MicroservicePlacementLogic microservicePlacementLogic = placementLogicFactory
						.getPlacementLogic(placementLogic, cdevice.getId());
				cdevice.initializeController(loadBalancer, microservicePlacementLogic,
						getResourceInfo(monitoredDevices), applications, monitoredDevices);
			} else if (cdevice.getDeviceType().equals(MicroserviceFogDevice.FCN)
					|| cdevice.getDeviceType().equals(MicroserviceFogDevice.CLIENT)) {
		...
		for (FogDevice f : fogDevices) {
			if (((MicroserviceFogDevice) f).getIsInCluster()) {
				for (int deviceId : ((MicroserviceFogDevice) f).getClusterMembers()) {
					Pair<Integer, Map<String, Double>> resources = new Pair<>(f.getId(),
							((MicroserviceFogDevice) f).getResourceAvailabilityOfDevice());
...
//                printQoSDetails();
	...
	protected void printCostDetails() {
		str.append(getCloud().getTotalCost());
		str.append(";");
	...
	protected void printNetworkUsageDetails() {
		str.append(Double.toString(NetworkUsageMonitor.getNetworkUsage() / Config.MAX_SIMULATION_TIME));
		str.append(";");
		System.out
...
//        }
	...
	protected static void createClusterConnections(int levelIdentifier, List<FogDevice> fogDevices,
	...
	protected void printClusterConnections() {
		...
		// <ParentNode,ClusterNodes> Assuming than clusters are formed among nodes with
		// same parent
			...
			for (MicroserviceFogDevice device : clusterNodes) {
				...
				for (Integer deviceId : device.getClusterMembers()) {
					if (!clusterNodes.contains(getFogDeviceById(deviceId))) {
						Logger.error("Cluster formation Error", "Error : " + getFogDeviceById(deviceId).getName()
								+ " is added as a cluster node of " + device.getName());
					}
				}
